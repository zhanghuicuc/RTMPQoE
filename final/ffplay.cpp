...
//自定义一个事件，用于定期检查缓存
#define QOE_CHECKIO_EVENT    (SDL_USEREVENT + 3)
//--------------------------------
//专门设置的标记，在程序将要退出的时候会置1
static int exit_remark=0;
//每隔一段时间，检查一下IO，单位us（10exp-6 s）
#define QOE_CHECK_TIME 100000
static int qoe_checkio_thread(void *opaque)
{
	int time_us=QOE_CHECK_TIME;
	VideoState *is= (VideoState *)opaque;
	//前提是AVFormatContext存在（即有视频播放）
	while (exit_remark==0) {
		//如果处于暂停状态，
		if(is->paused){
			av_usleep(time_us);
...
int qoe_rebuff_num;
//记录每次持续卡顿的时长
int continue_pause_time[MAX_IOCHECK_NUM];
//以下变量记录所有时间段的情况
//检测次数
//int qoe_checknum=0;
//持续卡顿次数
int qoe_continue_pause_num;
//总卡顿时间
int total_pause_time;
//用于算分的卡顿时间和次数
float total_pause_time_forscore;
float qoe_rebuff_num_forscore;

int pause_gap=0;
int pause_gap_all;
DWORD pause_gap_before;
DWORD pause_gap_after=0;
int pause_gap_cnt;

static void qoe_checkio(VideoState *is)
{
	AVFormatContext *pFormatCtx = is->ic;
	AVStream *stream=is->video_st;
	CString rebuff_num,pause_gap_str;

	CString buf_score_str;
	float buf_score;
	CString total_pause_time_str;
	...
	if(pFormatCtx->pb!=NULL)
	{
		...
		if((qoe_ptr_location_pre==buffer_ptr_location)&&(qoe_ptr_location_pre_val==*(pFormatCtx->pb->buf_ptr)))
		{
			if(is_rebuff==0){
				//首次出现暂停,则增加一次暂停次数
				qoe_rebuff_num++;
				time_pause_start=timeGetTime();
				//同时改编为暂停状态
				is_rebuff=1;
			}else{
				//不是首次暂停
				time_pause_last=timeGetTime();
				//卡顿时长
				time_last=time_pause_last-time_pause_start;//以毫秒为单位
				if(is_continue==0 && time_last>1)
				{
					qoe_continue_pause_num++;
					is_continue=1;
					continue_pause_time[qoe_continue_pause_num]=time_last;		
					total_pause_time+=continue_pause_time[qoe_continue_pause_num];
					dlg->charData_pause_time[qoe_continue_pause_num]=continue_pause_time[qoe_continue_pause_num];
				}
			}
		}else{
			//视频不再暂停
			...
			{	
				//卡顿间隔
				if(pause_gap_before==0)
					pause_gap_before=timeGetTime();
				else
				{
					pause_gap_cnt++;
					pause_gap_after=timeGetTime();
					pause_gap=pause_gap_after-pause_gap_before;
					pause_gap_before=0;
					pause_gap_all+=pause_gap;
					pause_gap_avg=pause_gap_all/pause_gap_cnt;
					pause_gap_str.Format("%d",pause_gap_avg);
					dlg->m_pause_gap.SetWindowTextA(pause_gap_str);
				}
				dlg->SeriesMarks.put_Visible(TRUE);
				dlg->barSeries0.AddXY((double)qoe_continue_pause_num,(double)dlg->charData_pause_time[qoe_continue_pause_num],NULL,RGB(255,0,0));
				//翻页
				dlg->page1.put_Current(dlg->page1.get_Count());
			}
			is_rebuff=0;
			is_continue=0;
		}
		...
		dlg->m_paused_num.SetWindowText(rebuff_num);
		//dlg->score->SetDlgItemTextA(IDC_D10_SCORE,d10_score_str);//这里用setwindowtext就会出错，需要注意
		
		//总卡顿时间
		total_pause_time_str.Format("%d",total_pause_time);
		dlg->m_paused_time.SetWindowText(total_pause_time_str);
		
		//卡顿得分30s检测一次
		if(play_sec!=0 && play_sec%30==0)
		{	
			total_pause_time_forscore=total_pause_time-total_pause_time_forscore;
			qoe_rebuff_num_forscore=qoe_rebuff_num-qoe_rebuff_num_forscore;
			buf_score=-0.0001361*total_pause_time_forscore-0.1073*qoe_rebuff_num_forscore+4.13;
			buf_score_str.Format("%5.2f",buf_score*20);
			dlg->score->m_buf_score_value=buf_score*20;
			dlg->score->m_score_buf.SetWindowText(buf_score_str);
		}
		if(play_sec!=0 && play_sec<30)
		{
			dlg->score->m_score_buf.SetWindowText("100");
			dlg->score->m_buf_score_value=100;
		}
...
int framerate_temp=0;
float resolution_temp=0;
//设置MFC参数，成功的话返回0，否则返回-1
//全局的，只设置一次
int qoe_param_global(VideoState *is){
	//decode_thread(is);
	CString framerate,resolution,pix_fmt,videocodec,
			audiochannel,audiosamplerate,audiocodec,
			format,input_protocol,metadata;
		
	format.Format("%s",is->ic->iformat->long_name);
	dlg->m_format.SetWindowText(format);
	
	videocodec.Format("%s",is->video_st->codec->codec->long_name);
	dlg->m_videocodec.SetWindowTextA(videocodec);

	
	if(is->ic->pb!=NULL)
	{
		URLContext *uc=(URLContext *)is->ic->pb->opaque;
	...
	resolution.Format("%d x %d",is->video_st->codec->width,is->video_st->codec->height);
	dlg->m_resolution.SetWindowText(resolution);	

	resolution_temp=is->video_st->codec->width * is->video_st->codec->height;

	framerate_temp=is->video_st->r_frame_rate.num/is->video_st->r_frame_rate.den;
	framerate.Format("%d fps",framerate_temp);
	...
	switch(is->video_st->codec->pix_fmt){
		case 0:
			...
			break;
		case 1:
			pix_fmt.Format("YUYV422");
			break;
		case 2:
			pix_fmt.Format("RGB24");
			break;
		case 3:
			pix_fmt.Format("BGR24");
			break;
		case 12:
			pix_fmt.Format("PIX_FMT_YUVJ420P");
			break;	
		default:
			pix_fmt.Format("UNKNOWN");
		}
		dlg->m_pixfmt.SetWindowText(pix_fmt);

	audiocodec.Format("%s",is->audio_st->codec->codec->long_name);
	dlg->m_audiocodec.SetWindowText(audiocodec);
	
	audiosamplerate.Format("%dhz",is->audio_st->codec->sample_rate);
	dlg->m_audiosamplerate.SetWindowText(audiosamplerate);

	audiochannel.Format("%d",is->audio_st->codec->channels);
	dlg->m_audiochannel.SetWindowTextA(audiochannel);
	if(is->audio_st->codec->channels==2)
	{
		dlg->score->m_score_audio.SetWindowTextA("100");
		dlg->score->m_audio_score_value=100;
	}
	else
	{
		dlg->score->m_score_audio.SetWindowTextA("50");
		dlg->score->m_audio_score_value=50;
	}
...
/*	m=av_dict_get(pFormatCtx->metadata,"author",m,0);
	...
	*/
	...
	while(m=av_dict_get(is->ic->metadata,"",m,AV_DICT_IGNORE_SUFFIX)){
		key.Format(m->key);
	...
	//pDlg->m_metadata.SetWindowText(author+"\r\n"+copyright+"\r\n"+description);
...
/*设置图表
int qoe_param_packet(VideoState *is,AVPacket *packet){
	//--------------------------------------------------------------------
	...
	if(is->ic->pb!=NULL)
	{
		...
		int buffer_size=is->ic->pb->buf_end-is->ic->pb->buffer;
		dlg->charData_buffer_size[packet_index]=buffer_size;
		dlg->lineSeries0.AddXY((double)packet_index,(double)dlg->charData_buffer_size[packet_index],
			NULL,0);
	}
	else
		 return -1;
	...
	//分页
	dlg->page0.put_Current(dlg->page0.get_Count());
...
float v_bitrate_all;
float bpp;
//视频帧参数提取,一是为了计算视频的码率，二是为了计算QP值
int qoe_param_vframe(VideoState *is,AVFrame *pFrame,AVPacket *packet){
	CString v_bitrate,bpp_str,video_score_str;
	float v_bitrate_temp=0;float v_bitrate_avg=0;float video_score;
	
	//算法用到的参数----------
	int MB_sum,mb_stride,mb_xy;
	long QP_sum = 0,QP_invalid_count = 0;
	CString Qp_sum;
	int mb_skip_sum=0;
	//全局的
	static int mb_skip_max=0,mb_skip_sum12f=0;
	static float QP_sum12f=0;
	double Icoding=0.0,Dppl=0.1681;//Icoding 编码损伤  Dppl 错误鲁棒性
	double score=0.0;
	static int i_num=0;
	//------------------------
	AVFormatContext *pFormatCtx = is->ic;
	...
	{
		case 1:
			gopsize_temp=vframe_index-gop;
			gop=vframe_index;
			gopsize.Format("%d",gopsize_temp);
			dlg->m_gopsize.SetWindowTextA(gopsize);
			break;
		default:
			break;
	}*/
	total_size+=packet->size;
	if(vframe_index!=0 && vframe_index%framerate_temp==0 && framerate_temp!=0)
	{
		play_sec++;
		dlg->play_sec_dlg=play_sec;
		v_bitrate_temp=(total_size*8)/play_sec;
		v_bitrate_all+=v_bitrate_temp;
		v_bitrate_avg=v_bitrate_all/play_sec;
		bpp=v_bitrate_avg/(resolution_temp*framerate_temp);
		v_bitrate.Format("%.2f kbps",v_bitrate_avg/1024);
		bpp_str.Format("%5.3f",bpp);
		dlg->m_bitrate.SetWindowTextA(v_bitrate);
		dlg->m_bpp.SetWindowTextA(bpp_str);

		dlg->charData_v_bitrate[play_sec]=v_bitrate_temp/1024;
		dlg->lineSeries_v_bitrate.AddXY((double)play_sec,(double)dlg->charData_v_bitrate[play_sec],
				NULL,0);

		video_score=1.316*log(bpp)+0.511*log(sqrt(resolution_temp*9/16))+2.946;
		dlg->score->m_video_score_value=video_score*20;
		video_score_str.Format("%5.2f",video_score*20);
		dlg->score->m_score_video.SetWindowTextA(video_score_str);
	}
	//分页
	dlg->page0.put_Current(dlg->page0.get_Count());
	...
	if(pFrame->pict_type == 1)
		i_num++;
	mb_stride = pCodecCtx->width/16+1;
	//MB_sum = (pCodecCtx->height/16+(pCodecCtx->height%16)&0x01)*mb_stride;
	MB_sum = (pCodecCtx->height/16)*mb_stride;
	for(mb_xy=0;mb_xy<MB_sum;mb_xy++)
	{
		if( (mb_xy+1) % mb_stride == 0)
		{
			//每行最右边那个宏块？不合法？？
			QP_invalid_count++;
			continue;
		...
		{
			//每个宏块都有一个Q值，把这些Q值加起来，Q的和
			QP_sum += pFrame->qscale_table[mb_xy];
			//是“跳过”类型的宏块时(MB_TYPE_SKIP:0x0800)
			//注意：必须判断mb_type是否被初始化了，在加载图像的时候，不会初始化mb_type数组！
			if(pFrame->mb_type!=NULL){
				if (pFrame->mb_type[mb_xy] & 0x0800)
				{
					//跳过的宏块数
					mb_skip_sum++;
				}
	...
	mb_skip_max = (mb_skip_max > mb_skip_sum) ? mb_skip_max:mb_skip_sum; 
	//-------------------------------
	//TRACE("average QP = %f\n",1.0*QP_sum/(MB_sum-QP_invalid_count));
	QP_sum12f += 1.0*QP_sum/(MB_sum-QP_invalid_count);
	mb_skip_sum12f += mb_skip_sum;
	if ((vframe_index)%12 == 0)
	{
		Qp_sum.Format("%5.2f",QP_sum12f/12);	
		dlg->m_qp.SetWindowTextA(Qp_sum);
	}
	if ((vframe_index)%12 == 0){
			QP_sum12f = 0;
			mb_skip_sum12f = 0;
...
int a_bitrate_cnt=0;
//音频帧参数提取
int qoe_param_aframe(VideoState *is,AVPacket *packet){
	//--------------------------------------------------------------------
	CString a_bitrate;
	int a_bitrate_temp,a_bitrate_avg;
	total_audio_size+=packet->size;
	if(vframe_index!=0 && vframe_index%framerate_temp==0 && framerate_temp!=0 &&play_sec!=0)
	{
		a_bitrate_cnt++;
		a_bitrate_temp=(total_audio_size*8)/(play_sec*1000);
		
		a_bitrate_all+=a_bitrate_temp;
		a_bitrate_avg=a_bitrate_all/a_bitrate_cnt;
		a_bitrate.Format("%d kbps",a_bitrate_avg);
		dlg->m_a_bitrate.SetWindowTextA(a_bitrate);
		
		dlg->charData_a_bitrate[play_sec]=a_bitrate_temp;
		dlg->lineSeries_a_bitrate.AddXY((double)play_sec,(double)dlg->charData_a_bitrate[play_sec],
				NULL,0);
	}
	//分页
	dlg->page0.put_Current(dlg->page0.get_Count());
...
static int video_open(VideoState *is, int force_set_video_mode)
{
	...
	//注意：设置视频窗口标题！
	//window_title="Internet Multimedia QoE Evaluation System (Video Window)";
    //if (!window_title)
...
static void stream_component_close(VideoState *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        is->audio_buf = NULL;
        avcodec_free_frame(&is->frame);////////////小玩意儿还挺重要，一删了全家都要卡死了
...
static int read_thread(void *arg)
{
	...
	//开启检查IO的线程---------------------
	SDL_CreateThread(qoe_checkio_thread, is);
	//--------------------------------
...
//以下是对“暂停”的封装
void ffplay_pause(VideoState *is){
	toggle_pause(is);
}
...
int alloc_cnt;
DWORD time_readyto_play;
DWORD time_really_play;
...
static void event_loop(VideoState *cur_stream)
{
	...
	//alloc_cnt=0;
	int buff_time=0;
	CString buff_time_str;
	CString access_score_str;
	float access_score;
    ...
    for (;;) {
       ...
       /* case SDL_KEYDOWN:
        ...
        case FF_ALLOC_EVENT:
			...
			if(alloc_cnt==0)
			{	alloc_cnt++;break;}
			if(alloc_cnt==1 && time_really_play==0)
			{
				time_really_play=timeGetTime();
				buff_time=time_really_play-time_readyto_play;
				if(dlg->dnsdelay1 && dlg->tcpdelay1 &&dlg->rtmpdelay1)
				{
					buff_time=buff_time*1000-dlg->dnsdelay1-dlg->tcpdelay1-dlg->rtmpdelay1;
					access_score=4.332-0.115*buff_time/1000000;
					buff_time_str.Format("%d",buff_time);
					access_score_str.Format("%5.2f",access_score*20);
					dlg->score->m_access_score_value=access_score*20;
					dlg->m_buff_time.SetWindowTextA(buff_time_str);
					dlg->score->m_score_access.SetWindowTextA(access_score_str);
					dlg->score->m_score_sync.SetWindowTextA("100");
				}
				break;
			}
        ...
        case FF_REFRESH_EVENT:
            ...
            break;
		case QOE_CHECKIO_EVENT:
			//定期检查IO
			qoe_checkio(cur_stream);
			break;
        default:
            break;
        }
...
int ffplay(LPVOID lpParam)
{
	...
	//播放时间
	 ...
	 is_rebuff=0;
	 qoe_rebuff_num=0;
	 qoe_continue_pause_num=0;
	 is_continue=0;
	 pause_gap_before=0;
	 pause_gap_cnt=0;
	 pause_gap_all=0;
	 total_pause_time=0;
	 total_pause_time_forscore=0;
	 qoe_rebuff_num_forscore=0;
	 memset(continue_pause_time,0,sizeof(continue_pause_time));
	total_size=0;
	v_bitrate_all=0;
	bpp=0;
	total_audio_size=0;
...
//    parse_loglevel(argc, argv, options);
   ...
   // init_opts();
	...
	//传入文件路径
	input_filename=(const char *)malloc(200);
	strcpy((char *)input_filename,dlg->rtmpurl);
	time_readyto_play=timeGetTime();
